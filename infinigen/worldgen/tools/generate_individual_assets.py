...
def build_scene(path, idx, factory_name, args):
    ...
    imgpath = path / f"image_{idx:03d}.png"
    ...
    if args.save_blend:
        ...
        blend_path = path / f"scene_{idx:03d}.blend"
        blend_path = str(blend_path)
        bpy.ops.wm.save_as_mainfile(filepath=blend_path, filter_backup=True)
        tag_system.save_tag(f"{path}/MaskTag.json")

        meta_dict = {'displayName': factory_name.replace("_", " "), 'tag': ['procedural'], 'author': 'infinigen', "assetType": "material",}
        meta_path = path / f"meta.json"
