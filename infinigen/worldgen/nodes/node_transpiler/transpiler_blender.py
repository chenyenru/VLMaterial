...
GRAPH_ONLY = False
RESERVE_ORDER = True
UNGROUP_NODE_GROUP = True
SIMPLE_FUNC_NAME = True
...
def represent_default_value(val, simple=True):
    ...
    if isinstance(val, (str, int, bool, bpy.types.Object, bpy.types.Collection, set, bpy.types.Image)):
        ...
        if USE_PSEUDO_CODE or ROUND_NUMBER:
            code = f'{round(val, 2)}'.removesuffix('.0')
        else:
            code = f'{val:.4f}'
...
def special_case_colorramp(node, varname):
    ...
    for i, ele in enumerate(cramp.elements):
        code += f'{varname}.color_ramp.elements[{i}].position = {represent_default_value(ele.position)}\n'
        code += f'{varname}.color_ramp.elements[{i}].color = {represent_list(ele.color)}\n'

    return code

def special_case_colorramp_kwargs(node):
    assert node.bl_idname == Nodes.ColorRamp
    cramp = node.color_ramp
    kwargs = {}
    if cramp.interpolation != 'LINEAR': # dont bother if left at default
        kwargs['interpolation'] = repr(cramp.interpolation)
    kwargs['positions'] = [ele.position for ele in cramp.elements]
    kwargs['colors'] = [tuple(ele.color) for ele in cramp.elements]
    return kwargs
...
def special_case_curve(node, varname):
    ...
    for i, c in enumerate(node.mapping.curves):
        for j, p in enumerate(c.points):
            if j < 2:
                if j == 0 and tuple(p.location) == (0, 0):
                    continue
                if j == 1 and tuple(p.location) == (1, 1):
                    continue
                code += f'{varname}.mapping.curves[{i}].points[{j}].location = {represent_tuple(p.location)}\n'
            else:
                code += f'{varname}.mapping.curves[{i}].points.new({p.location[0]}, {p.location[1]})\n'

            if p.handle_type != 'AUTO':
                code += f'{varname}.mapping.curves[{i}].points[{j}].handle_type = {repr(p.handle_type)}\n'
    return code

def special_case_curve_kwargs(node):
    assert node.bl_idname in [Nodes.FloatCurve, Nodes.RGBCurve, Nodes.VectorCurve]
    kwargs = {}
    curve_points = []
    for i, c in enumerate(node.mapping.curves):
        points = [list(p.location) for p in c.points]
        curve_points.append(points)
    if len(curve_points) > 0:
        kwargs['curve_points'] = curve_points
    return kwargs
...
def special_case_value_kwargs(node):
    ...
    kwargs = {}
    if node.bl_idname in [Nodes.Value, Nodes.RGB]:
        kwargs['value'] = value_expr
    elif node.bl_idname == Nodes.Vector:
        kwargs['vector'] = value_expr
    elif node.bl_idname == Nodes.InputColor:
        kwargs['color'] = value_expr
    elif node.bl_idname == Nodes.Integer:
        kwargs['integer'] = value_expr
    else:
        raise ValueError(f'special_case_value called on unrecognized {node.bl_idname=}')
...
def create_inputs_dict(node_tree, node, memo):
    ...
    for i, (input_name, input_socket) in enumerate(node.inputs.items()):
        ...
        if links is None:
            if GRAPH_ONLY:  # skip specific values if we only want the graph
                continue
        ...
        if ONLY_BASE_COLOR:
            if (node.bl_idname == Nodes.PrincipledBSDF and input_name != 'Base Color') or \
                (node.bl_idname == Nodes.DiffuseBSDF and input_name != 'Color') or \
                (node.bl_idname == Nodes.GlassBSDF and input_name != 'Color') or \
                (node.bl_idname == Nodes.GlossyBSDF and input_name != 'Color') or \
                (node.bl_idname == Nodes.RefractionBSDF and input_name != 'Color') or \
                (node.bl_idname == Nodes.TranslucentBSDF and input_name != 'Color') or \
                (node.bl_idname == Nodes.TransparentBSDF and input_name != 'Color'):
                continue
        ...
        for link in links:
            ...
            # code += input_code
            code_nodes = append_code(code_nodes, input_code[0])
            code_links = append_code(code_links, input_code[1])
            code_params = append_code(code_params, input_code[2])
            new_transpile_targets.update(targets)

            if len(link.from_node.outputs) == 1:
                input_expression = f"{input_varname}.outputs[0]"
            else:
    ...
    if USE_PSEUDO_CODE:
        chars = ['a', 'b', 'c', 'd', 'e']
        for i in range(5):
            if i in inputs_dict.keys():
                inputs_dict[chars[i]] = inputs_dict.pop(i)
        if node.bl_idname == Nodes.ColorRamp:
            inputs_dict.update(special_case_colorramp_kwargs(node))
        elif node.bl_idname in (Nodes.FloatCurve, Nodes.RGBCurve, Nodes.VectorCurve):
            inputs_dict.update(special_case_curve_kwargs(node))
        elif node.bl_idname in VALUE_NODES:
            inputs_dict.update(special_case_value_kwargs(node))

    return inputs_dict, (code_nodes, code_links, code_params), new_transpile_targets
...
def represent_kwargs(inputs_dict, spacing=' '):
    vals = []
    for k, v in inputs_dict.items():
        if isinstance(k, str):
            k = k.replace(' ', '').strip("'")
        vals.append(f'{k}={repr_iter_val(v)}')
    vals = f',{spacing}'.join(vals)
    return vals
...
def get_varname(node, taken):
    ...
    # special case if the name is still empty
    if not name:
        name = 'node'
...
def replace_undefined_node(node_tree, old_node):
    """Replace an old node with a new one in a material."""
    links = node_tree.links

    node_input_ids = [i.identifier for i in old_node.inputs]
    if node_input_ids == ['Factor_Float', 'Factor_Vector', 'A_Float', 'B_Float', 'A_Vector', 'B_Vector', 'A_Color', 'B_Color']:
        new_node_type = Nodes.MixRGB

        input_id_map = {
            'Factor_Float': 'Fac', 'A_Color': 'Color1', 'B_Color': 'Color2',
        }
        output_id_map = {
            'Result_Color': 'Color'
        }
        attrs_map = {
            'use_alpha': 'use_alpha', 'blend_type': 'blend_type', 'use_clamp': 'use_clamp'
        }
    else:
        raise ValueError(f"replace_undefined_node could not handle {old_node.bl_idname} with inputs {node_input_ids}")

    # Create a new node
    new_node = node_tree.nodes.new(type=new_node_type)
    new_node.location = (old_node.location.x, old_node.location.y)

    # Replace input values and connections
    for input in old_node.inputs:
        if input.identifier not in input_id_map:
            continue
        new_input_name = input_id_map[input.identifier]
        if len(input.links) == 0:
            new_node.inputs[new_input_name].default_value = input.default_value
        else:
            for link in input.links:
                # print(f"link {link.from_node.name}.{link.from_socket.name} to {link.to_node.name}.{link.to_socket.name}")
                links.new(new_node.inputs[new_input_name], link.from_socket)

    # Replace output connections
    for output in old_node.outputs:
        if output.identifier not in output_id_map:
            continue
        for link in output.links:
            new_output_name = output_id_map[output.identifier]
            # print(f"link {link.from_node.name}.{link.from_socket.name} to {link.to_node.name}.{link.to_socket.name}")
            links.new(link.to_socket, new_node.outputs[new_output_name])
    ...
    #         continue
    #     setattr(new_node, attrs_map[attr], getattr(old_node, attr))


    print(f"replace {old_node.bl_idname} with {new_node.bl_idname}")
    # Remove the old node
    node_tree.nodes.remove(old_node)


def copy_node_attrs(node, new_node):
    assert node.bl_idname == new_node.bl_idname

    # copy valued inputs
    for input_key in node.inputs.keys():
        if len(node.inputs[input_key].links) == 0 and hasattr(node.inputs[input_key], 'default_value'):
            new_node.inputs[input_key].default_value = node.inputs[input_key].default_value

    # copy attributes
    for attr in node_attrs_available(node):
        setattr(new_node, attr, getattr(node, attr))

    # special case for colorramp
    if node.bl_idname == Nodes.ColorRamp:
        new_node.color_ramp.interpolation = node.color_ramp.interpolation
        extra_n_elements = len(node.color_ramp.elements) - len(new_node.color_ramp.elements)
        extra_n_elements = max(0, extra_n_elements)
        for _ in range(extra_n_elements):
            new_node.color_ramp.elements.new(0)

        for i, ele in enumerate(node.color_ramp.elements):
            new_node.color_ramp.elements[i].position = ele.position
            new_node.color_ramp.elements[i].color = ele.color

    # special case for curve
    elif node.bl_idname in [Nodes.FloatCurve, Nodes.RGBCurve, Nodes.VectorCurve]:
        for i, c in enumerate(node.mapping.curves):
            extra_n_points = len(c.points) - len(new_node.mapping.curves[i].points)
            extra_n_points = max(0, extra_n_points)
            for _ in range(extra_n_points):
                new_node.mapping.curves[i].points.new(0, 0)
            for j, p in enumerate(c.points):
                new_node.mapping.curves[i].points[j].location = p.location
                new_node.mapping.curves[i].points[j].handle_type = p.handle_type
                # new_node.mapping.curves[i].points[j].interpolation = p.interpolation

    # special case for value
    elif node.bl_idname in [Nodes.Value, Nodes.RGB]:
        new_node.outputs[0].default_value = node.outputs[0].default_value
    elif node.bl_idname in Nodes.Vector:
        new_node.vector = node.vector
    elif node.bl_idname == Nodes.InputColor:
        new_node.color = node.color
    elif node.bl_idname == Nodes.Integer:
        new_node.integer = node.integer


def remove_all_reroutes(node_tree):
    # List of reroute nodes to be removed
    reroute_nodes = [node for node in node_tree.nodes if node.bl_idname == 'NodeReroute']

    for reroute_node in reroute_nodes:
        # Identify nodes and sockets linked to the reroute
        input_links = [link for link in node_tree.links if link.to_node == reroute_node]
        output_links = [link for link in node_tree.links if link.from_node == reroute_node]
        ...
        node_tree.nodes.remove(reroute_node)
    return node_tree


def ungroup_node_group(node_tree, node_group, recursive=False):
    if node_group.type != 'GROUP':
        print("Node is not a group!")
        return

    if node_group.node_tree is None:
        print("Node group has no node tree!")
        node_tree.nodes.remove(node_group)
        return

    # select node_group to be active
    # node_group.select = True
    # bpy.ops.node.group_ungroup()

    # Mapping for created nodes
    new_nodes = {}

    node_group.node_tree = remove_all_reroutes(node_group.node_tree)

    for node in node_group.node_tree.nodes:
        if node.bl_idname == 'NodeUndefined':
            replace_undefined_node(node_group.node_tree, node)

    # Create new nodes based on internal nodes
    for node in node_group.node_tree.nodes:
        ...
        # new_node.location = (node_group.location.x + node.location.x, node_group.location.y + node.location.y)
        new_nodes[node.name] = new_node
        copy_node_attrs(node, new_node)

    # Copy internal links to new nodes
    for link in node_group.node_tree.links:
        if link.from_node.bl_idname == 'NodeGroupInput' and link.to_node.bl_idname == 'NodeGroupOutput':
            if len(node_group.outputs[link.to_socket.name].links) == 0:
                # not used
                continue
            else:
                for out_link in node_group.outputs[link.to_socket.name].links:
                    for input_link in node_group.inputs[link.from_socket.name].links:
                        node_tree.links.new(input_link.from_socket, out_link.to_socket)
        elif link.from_node.bl_idname == 'NodeGroupInput':
            if len(node_group.inputs[link.from_socket.name].links) == 0:
                src_socket = node_group.inputs[link.from_socket.name]
                dst_socket = new_nodes[link.to_node.name].inputs[link.to_socket.name]
                # print("src_socket", tuple(src_socket.default_value), isinstance(src_socket.default_value, (tuple)))
                # print("dst_socket", tuple(dst_socket.default_value))
                if not hasattr(src_socket, 'default_value') or not hasattr(dst_socket, 'default_value'):
                    if hasattr(src_socket, 'default_value') != hasattr(dst_socket, 'default_value'):
                        raise RuntimeError(f"Source socket type '{src_socket.bl_idname}' does not match "
                                           f"the target '{dst_socket.bl_idname}'")
                    continue
                if isinstance(dst_socket.default_value, (int, float, bool)) and isinstance(src_socket.default_value, (int, float, bool)):
                    dst_socket.default_value = src_socket.default_value
                elif isinstance(dst_socket.default_value, (int, float, bool)):
                    dst_socket.default_value = src_socket.default_value[0]
                elif isinstance(src_socket.default_value, (int, float, bool)):
                    dst_socket.default_value = tuple([src_socket.default_value] * len(dst_socket.default_value))
                else:
                    min_len = min(len(dst_socket.default_value), len(src_socket.default_value))
                    dst_socket.default_value = tuple(src_socket.default_value[:min_len])
            else:
                for input_link in node_group.inputs[link.from_socket.name].links:
                    node_tree.links.new(input_link.from_socket,
                                        new_nodes[link.to_node.name].inputs[link.to_socket.name])
        elif link.to_node.bl_idname == 'NodeGroupOutput':
            for out_link in node_group.outputs[link.to_socket.name].links:
                node_tree.links.new(new_nodes[link.from_node.name].outputs[link.from_socket.name],
                                    out_link.to_socket)
        else:
            node_tree.links.new(new_nodes[link.from_node.name].outputs[link.from_socket.name],
                                new_nodes[link.to_node.name].inputs[link.to_socket.name])
...
# def create_node_group(target, memo):
#     output_node_id = OUTPUT_NODE_IDS[type(target)]
#     node_tree = get_node_tree(target)

#     # replace undefined nodes with defined ones
...
def create_node(node_tree, node, memo):
    ...
    if idname == 'NodeUndefined':
        replace_undefined_node(node_tree, node)

    if IGNORE_REROUTES and idname == Nodes.Reroute:
        print("node tree:", node_tree)
        print(node, node.name, node.inputs)
        print(node.inputs[0])
        _links = get_connected_link(node_tree, node.inputs[0])
        link = _links[0] if _links else None
    ...
    if node.bl_idname.endswith('NodeGroup'):
        raise NotImplementedError('Node groups are not yet supported')
        ...
        group_code, memo = create_node_group(node, memo)
        code_nodes = append_code(code_nodes, group_code[0])
        code_links = append_code(code_links, group_code[1])
        code_params = append_code(code_params, group_code[2])
        nodetype_expr = f'{funcname}().name'
    elif node.bl_idname == 'ShaderNodeScript':
        raise NotImplementedError('ShaderNodeScript is not yet supported')
    else:
        nodetype_expr = get_nodetype_expression(node)
    if USE_PSEUDO_CODE:
        nodetype_expr = nodetype_expr.strip("'").replace('Nodes.', '').replace('ShaderNode', '')
    new_node_args.append(nodetype_expr)

    varname = get_varname(node, taken=list(memo.values()))
    memo[node.name] = varname
    # print(node.bl_idname, node.name, varname)
    ...
    #     if USE_PSEUDO_CODE:
    #         kwargs_str = represent_kwargs(inputs_dict)
    #     else:
    #         kwargs_str = f'input_kwargs={represent_dict(inputs_dict)}'
    #     new_node_args.append(kwargs_str)
    ...
    # if len(attrs_dict) > 0:
    #     if USE_PSEUDO_CODE:
    #         attrs_str = represent_kwargs(attrs_dict)
    #     else:
    #         attrs_str = f'attrs={represent_dict(attrs_dict)}'
    #     new_node_args.append(attrs_str)
    ...
    #     arg_sep = ', '
    # if USE_PSEUDO_CODE:
    #     new_node_args_str = arg_sep.join(new_node_args[1:])
    #     code += f'{varname} = {new_node_args[0]}({new_node_args_str})\n'
    # else:
    #     new_node_args_str = arg_sep.join(new_node_args)
    #     code += f'{varname} = nw.new_node({new_node_args_str})\n'
    code_nodes += f"{varname} = nodes.new('{node.bl_idname}')\n"

    for k, v in inputs_dict.items():
        if isinstance(v, str) and 'outputs' in v: # link
            if RESERVE_ORDER:
                code_links = append_code(code_links, f"links.new({varname}.inputs[{k}], {v})\n")
            else:
                code_links = append_code(code_links, f"links.new({v}, {varname}.inputs[{k}])\n")
            # code_links += f"links.new({v}, {varname}.inputs[{k}])\n"
        else:
            code_params = append_code(code_params, f"{varname}.inputs[{k}].default_value = {v}\n")
            # code_params += f"{varname}.inputs[{k}].default_value = {v}\n"

    for k, v in attrs_dict.items():
        code_params = append_code(code_params, f"{varname}.{k[1:-1]} = {v}\n")
        # code_params += f"{varname}.{k[1:-1]} = {v}\n"

    # Handle various special case nodes that dont behave like the others
    if not GRAPH_ONLY:
        if node.bl_idname == Nodes.ColorRamp:
            if not USE_PSEUDO_CODE:
                code_params = append_code(code_params, special_case_colorramp(node, varname))
                # code_params += special_case_colorramp(node, varname)
        elif node.bl_idname in (Nodes.FloatCurve, Nodes.RGBCurve, Nodes.VectorCurve):
            if not USE_PSEUDO_CODE:
                code_params = append_code(code_params, special_case_curve(node, varname))
                # code_params += special_case_curve(node, varname)
        elif node.bl_idname in VALUE_NODES:
            if not USE_PSEUDO_CODE:
                code_params = append_code(code_params, special_case_value(node, varname))
                # code_params += special_case_value(node, varname)

    code_nodes = append_code(code_nodes, inputs_code[0])
    code_links = append_code(code_links, inputs_code[1])
    code_params = append_code(code_params, inputs_code[2])

    # memo[node.name] = varname
    return varname, (code_nodes, code_links, code_params), new_transpile_targets
...
def write_function_body(target):
    ...
    node_tree = remove_all_reroutes(node_tree)

    if UNGROUP_NODE_GROUP:
        for node in node_tree.nodes:
            if node.bl_idname == 'ShaderNodeGroup':
                ungroup_node_group(node_tree, node)
                node_tree = get_node_tree(target)
        node_tree = get_node_tree(target)
...
def get_func_name(target):
    ...
    if category == "shader" and SIMPLE_FUNC_NAME:
        finalname = f'{category}_material'
    else:
        finalname = f'{category}_{name}'
    finalname = finalname.replace('__', '_').strip('_')
    finalname = finalname.replace('-', '_').strip('_')

    # remove starting numbers
    while len(finalname) > 0 and finalname[0].isdigit():
...
def transpile(orig_targets, module_dependencies=[]):
    ...
    while any(not v[1] for v in targets.values()):
        ...
        if not USE_PSEUDO_CODE:
            new_code += f'def {funcname}(material: bpy.types.Material):\n'
        else:
            new_code += f'def {funcname}():\n'

        # if not USE_PSEUDO_CODE and INCLUDE_IMPORTS:
        ...
        func_prefix = f'material.use_nodes = True\nnodes = material.node_tree.nodes\nlinks = material.node_tree.links\n\n'
        function_body = func_prefix + function_body
...
def transpile_object(obj, module_dependencies=[]):
    ...
    if USE_PSEUDO_CODE:
        ...
        code += func_code
        code += ('\n' + postfix(funcnames, targets)) if INCLUDE_POSTFIX else ''
